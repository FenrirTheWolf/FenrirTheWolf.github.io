...
document.getElementById("versionnum").innerHTML = "0.44" + patchversion;
...
for (let vxc in undum.game.newhosts.names) {
    ...
    $.extend(undum.game.situations.ch2.actions, {
        ["host" + namey]: function(character, system, action) {
    ...
    for (let ohdearno in undum.game.newhosts[undum.game.newhosts.names[vxc]].scenes) {
        ...
        for (let kc = 1; kc < Object.keys(simpscene).length + 1; kc++) {
            ...
            if (simpscene["scene" + kc].length == 3) {
                $.extend(situationholder[namey + "host" + ohdearno].actions, {
                    ["scene" + kc]: function(character, system, action) {
                        transer();
                        //system.setQuality("timeofday", character.qualities.timeofday + 1);
                        //time is NOW AFTERNOON
                        // all three here. head sticking out sheath OR bret carrying you in harness
...
undum.game.qualities = {
    ...
    luck: new undum.FudgeAdjectivesQuality( // Fudge as in the FUDGE RPG
        "<span title='Skill, Stamina and Luck are reverently borrowed from the Fighting Fantasy series of gamebooks.).'>Luck</span>", { priority: "0003", group: 'stats' }
    ),
