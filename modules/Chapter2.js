                    ...
                    try {
                        ...
                        if (character.sandbox.holdem) {
                            ...
                            for (x2 in character.sandbox.newhostnames) {
                                var loclist = character.nh[tempnames[x2]]["locations"];
                                if (typeof loclist === "string" && !fulllist.includes(loclist) && !newhostlocations.hasOwnProperty(loclist)) {
                                    fulllist.push(loclist);
                                } else {
                                    for (x2 in loclist){
                                        if (typeof loclist[x2] === "string" && !fulllist.includes(loclist[x2]) && !newhostlocations.hasOwnProperty(loclist[x2])) {
                                            fulllist.push(loclist[x2]);
                                        }
                                    }
                                }
                            }
                            for (x2 in fulllist) { 
                                if (!newhostlocations.hasOwnProperty(fulllist[x2])) {
                                    newhostlocations[fulllist[x2]] = [];
                                    console.log("Found a new location: '" + fulllist[x2] + "'");
                                }
                            }
                            if (fulllist.length === 1) {
                                console.log("Location added: (" + fulllist[0] + ")");
                            } else {
                                console.log("Locations added: (" + fulllist + ")");
                            }
                            character.sandbox.ch2hostloc = newhostlocations;
                            console.log("New locations list:", character.sandbox.ch2hostloc);
        ...
        "", {
            ...
            actions: {
                ...
                "finishdrink": function (character, system, action) {
                    transer();
                    parer("You quickly down the drink not giving it any chance to escape. Your eyes nearly cross as you clench your entire body just like when you were a cock... It doesn't take long before the room gets a bit funny feeling...");
                ...
                "isdrunk": function (character, system, action) {
                    transer();
                    parer("This is great! The music the dancing! For the first time in days you are not thinking anything naughty at all. Just enjoying yourself before realising you have been rubbing your butt against someone for the last minute. You feel a bit embarrassed and end up heading back to the bar to sit down...");
                    ch2hoster(character, system);
                    ch2menuer(character,system);
                }, //function close
                "out": function (character, system, action) {
...
herer = function(character, localloc){
    ...
    try {
        if (perlist.length == 1) {
            //one person here
            console.log("Just one character: " + perlist);
            ...
            //more than one  
            console.log("More than one character: " + perlist );
        ...
        console.log("There's a " + roommention );
        return roommention;
    } catch (e) {
...
ch2header = function(character, loc) {
    console.log("Available location: " + loc);
    ...
    if (character.sandbox.ch2beento.includes(loc) || character.sandbox.listmode) {
        for (let personhere in perlist) {
            ...
            if (character.nh[perlist[personhere]].known) {
                ...
                console.log("You know " + returnlist);
            } else {
                returnlist += character.nh[perlist[personhere]].species;
                console.log("There's a " + returnlist);
            }
            returnlist += " ";
        }
    } else {
