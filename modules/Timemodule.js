...
function timemod(system, character, opts,timey){
  ...
  setTimeout(function (system,character,opts,timey) {
    ...
    }, (timey)?timey:character.sandbox.eventspeed,system,character,opts,timey);
  }
...
//console.log("testing "+person[x]+" to replace "+arrarer[pos]); 
  ...
  for (x in person) {
    if (x=="fur"){
      ...
      res = res.replace(new RegExp(" fur ", 'g'), " scales ");
      res = res.replace(new RegExp(" fur. ", 'g'), " scales. ");
    }else{
...
var reptext=girlifier[x][1];
var reptext2=girlifier[x][0];

    res = res.replace(new RegExp(reptext, 'g'),reptext2);
//console.log("testing "+person[x]+" to replace "+arrarer[pos]); 
